...
export function ContactForm() {
  ...
  const hasAnyContact =
    state.phone.trim().length >= 7 || state.email.trim().length > 3;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!state.consent) {
      setError("Zaznacz zgodę na przetwarzanie danych (RODO).");
      return;
    }
    ...
    try {
      ...
      setError(
        err?.message || "Wystąpił błąd serwera. Spróbuj ponownie później."
      );
    } finally {
      setLoading(false);
    }
  ...
  if (submitted) {
    return (
      <div className="py-10 text-center">
        ...
        <h3 className="mb-2 text-xl font-bold">Wiadomość wysłana</h3>
        <p className="text-pretty text-muted-foreground">
          Dzięki za wiadomość. Odpowiem tak szybko, jak się da. Jeśli podałeś
          telefon, oddzwonię.
        </p>
      </div>
    );
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Wystarczy telefon lub e-mail. Nie oba.
          </p>
        </div>
        <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="Wycena ubezpieczenia">
              Wycena ubezpieczenia
            </SelectItem>
            <SelectItem value="Przedłużenie polisy">
              Przedłużenie polisy
            </SelectItem>
            <SelectItem value="Szkoda / assistance">
              Szkoda / assistance
            </SelectItem>
            <SelectItem value="Ubezpieczenie firmowe">
      ...
      <div className="flex items-start gap-3">
        <Checkbox
          ...
          onCheckedChange={(v) =>
            setState((s) => ({ ...s, consent: Boolean(v) }))
          }
        />
